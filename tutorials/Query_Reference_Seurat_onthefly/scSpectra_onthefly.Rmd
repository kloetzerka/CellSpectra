---
title: "scSpectra: Query - Reference analysis from a Seurat object"
...
scSpectra compares the gene expression pattern of a function or pathway in a 
...
The following example shows the "gold standard" in scSpectra analysis starting 
from a Seurat object. 
We realised that differences in cell numbers / expression sparsity can have an 
impact on spectra results. This can be significantly improved by performing an 
"onthefly" analysis. Thereby, reference samples match the total counts of each 
query individual query sample.
...
To run the "onthefly" version of scSpectra we need two things:

1.) A seurat object storing our single cell raw counts of query and reference 
samples

scSpectra expects specific metadata columns within this object and raw counts 
accesible as a matrix. 
We will prepare the object with one of the scSpectra functions to make sure 
everything runs smooth.

2.) A gene set database with genes grouped into funtionally-related pathways or 
functions.

scSpectra expects the gene sets in a binary matrix of genes (rows) and gene sets 
(columns) with 0 and 1 indicating if a gene belongs to a gene set. You can 
import this matrix yourself or you use one of the scSpectra functions to create 
such a matrix.

```{r load seurat object, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
#here we use the example ifnb dataset from Seurat.data to show basic functions
seurat_object <- readRDS("ifnb.rds")

#now we adapt the metadata columns. scSpectra needs to find the cell type 
...
```{r prepare gene sets, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
#prepare database
process_gene_sets(
...
```{r create reference, eval=FALSE, include=FALSE}
#create references
create_references(
  seurat_object = seurat_object,
  output_folder_base = output_folder_base,
...
Note: In a real world dataset scSpectra benefits from a wide and heterogenous 
...
from the scpectra analysis will save resources (creating references is memory 
intensive, running spectra is not).

The following function we compute fdr corrected p values and R2 values for each 
cell type, query sample, and gene set.

```{r run spectra, eval=FALSE, include=FALSE}

run_spectra(
  output_folder_base = output_folder_base,
  cell_types = c("CD14 Mono", "CD4 Naive T", "CD4 Memory T"), #evtl. parallelize
...
We hope this helps to run Spectra "onthefly" starting from a Seurat object 
and an enrichR library text file of any database. It's also possible to run 
spectra from any (pseudo) bulk matrix without "onthefly" reference generation. 
